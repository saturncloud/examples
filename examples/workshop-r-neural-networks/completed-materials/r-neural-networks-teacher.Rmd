...
output <-  # teach only
  input %>%
  layer_lstm(units = 24, return_sequences = TRUE) %>% # We tried adjusting to 32 units and adding an extra layer
  layer_lstm(units = 24, return_sequences = FALSE) %>%
  layer_dropout(rate = 0.05) %>% # Try adjusting this up to 0.2 and removing
...
input_v2_name <- layer_input(shape = c(max_length,num_characters)) 
input_v2_species <- layer_input(shape = 1)

intermediate_v2_name_processing <-
  input_v2_name %>%
...
output_v2 <- 
  layer_concatenate(list(intermediate_v2_name_processing, input_v2_species)) %>%
  layer_dense(num_characters) %>%
  layer_activation("softmax")

model_v2 <- 
  keras_model(inputs = list(input_v2_name, input_v2_species), outputs = output_v2) %>% 
  compile(
    loss = 'categorical_crossentropy',
    optimizer = "adam",
    metrics = c('accuracy','mse')
...
pred_input <- layer_input(shape = c(x_max_length,num_characters)) 

pred_output <-  # teach only
  pred_input %>%
  layer_lstm(units = 32, return_sequences = TRUE) %>%
  layer_lstm(units = 32, return_sequences = TRUE) %>%
  layer_lstm(units = 32, return_sequences = FALSE) %>%
...
pred_model <- keras_model(inputs = pred_input, outputs = pred_output) %>% 
  compile(
