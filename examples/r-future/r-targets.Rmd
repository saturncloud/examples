...
First, we need to create a file named "_targets.R". This file will contain all the information for targets to create a workflow.

We do this using `writeLines` for simplicity, but you could also use [Target Markdown](https://books.ropensci.org/targets/markdown.html) to accomplish a similar task.
...
Below is an example of the DAG for this workflow. As you can see, each node is connected and has a status.
...
That's it! This particular workflow takes approximately five minutes to compute all the hyperparameter options.
...
## Conclusion
The targets package is fantastic for writing pipeline code. It has the added benifit of running on parallel backends like future.  
